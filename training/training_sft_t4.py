"""Colab / T4 SFT: Unsloth 4-bit LoRA with TRL SFTTrainer + formatting_func.

Google Colab T4 (16 GB) and similar GPUs: FP16 training, modest sequence length,
no multiprocess dataset workers by default. Message-format JSONL requires
``formatting_func`` for current TRL + Unsloth stacks.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T4/Colab CORP-ENV SFT (Unsloth + formatting_func).")
    ...
    parser.add_argument("--lora-r", type=int, default=16, help="LoRA rank (16 is safer on 16 GB VRAM).")
    ...
    g.add_argument("--fp16", dest="fp16", action="store_true", help="Train in FP16 (default on T4).")
    g.add_argument("--bf16", dest="fp16", action="store_false", help="Train in BF16 when GPU supports it.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Dataset preprocess workers; 0 disables where TRL supports None.",
    )
    ...
    try:
        ...
        raise SystemExit("SFT requires datasets, trl, and unsloth.") from exc
    ...
    if not data_path.is_file():
        raise SystemExit(f"Training data not found: {data_path.resolve()}")
